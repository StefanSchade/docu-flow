...
def pipeline_manager():
    return PipelineManager(config_file='/workspace/src/configs/pipeline_config.json', data_dir='workspace/test_data')

# Asserts that the Pipelinemanager...

# ...can verify dependencies for a pipeline step
def test_check_dependencies(pipeline_manager):
    assert pipeline_manager.check_dependencies("PreprocessStep") == True
...
def test_verify_outputs(pipeline_manager, tmpdir):
    ...
    assert pipeline_manager.verify_outputs([str(output_file)]) == True

# ...fails to verify a missiong output
def test_missing_output(pipeline_manager):
    assert pipeline_manager.verify_outputs(["nonexistent_file.txt"]) == False
